...
# Configuration du Limiteur de requêtes par IP
limiter = Limiter(
    get_remote_address,
    app=app,
...
# 2. Constantes et Mappings globaux
...
USED_CODES = set()
ACCESS_CODES = {
    ADMIN_CODE: {"used": False, "created_at": "2026-09-16"}
}
...
def verify_user_pin(user_code, input_pin):
    if not user_code or not input_pin:
        return False
    expected_pin = calculate_expected_pin(user_code)
    return expected_pin == input_pin.strip().upper()

def generate_unique_code(prefix="FAS"):
    ...
    while True:
        ...
        if code not in ACCESS_CODES:
            ACCESS_CODES[code] = {"used": False, "created_at": datetime.now().isoformat()}
            return code

# 4. Middlewares et gestionnaires d'erreurs
@app.before_request
...
def verify_pin():
    ...
    input_pin = data.get('input_pin', '').strip().upper()

    if not user_code or not input_pin:
        return jsonify({"success": False, "message": "Kòd ak PIN obligatwa."}), 400

    if user_code in USED_CODES:
        return jsonify({"success": False, "message": "Kòd sa a deja sèvi yon fwa!"}), 400

    if verify_user_pin(user_code, input_pin):
        return jsonify({"success": True, "message": "Kòd valide ak siksè!"})
    else:
        return jsonify({"success": False, "message": "PIN an pa bon. Verifye sa w resevwa sou WhatsApp la."}), 400
...
def generate_report():
    ...
    # 1. Vérification du code d'accès / PIN
    if not access_code:
    ...
    if access_code == ADMIN_CODE:
        ...
        if access_code in USED_CODES:
            return jsonify({"error": "Ce code a déjà été utilisé pour générer un rapport."}), 400
    ...
    lang_instruction = "Rédige le rapport intégralement en Français." if lang == 'fr' else "Ekri tout rapò a nèt nan lang Kreyòl Ayisyen."
...
"""
    try:
        ...
        # 5. Marquage du code comme utilisé (si non admin)
        if access_code != ADMIN_CODE:
            USED_CODES.add(access_code)
